...
from ultrasync.jit import FileTracker, SymbolRecord
...
def directory_option(help_text: str = "Directory with .ultrasync index"):
    return click.option(
        "-d",
        "--directory",
...
@click.version_option(package_name="ultrasync")
def cli():
...
def symbols(directory: Path | None, name_filter: str | None):
    ...
    if not data_dir.exists():
        ...
        click.echo("run 'ultrasync index <directory>' first", err=True)
        sys.exit(1)
...
def voyager(directory: Path | None, model: str, no_classify: bool):
    ...
    try:
        from ultrasync.voyager import check_textual_available, run_voyager
    except ImportError:
        click.echo("error: textual is required for voyager TUI", err=True)
        click.echo("install with: pip install ultrasync[voyager]", err=True)
        sys.exit(1)
...
def mcp(
    ...
    from ultrasync.mcp_server import run_server
    ...
    if transport != "stdio":
        click.echo(
            f"starting ultrasync MCP server (transport={transport})..."
        )
        if root:
...
def keys(
    ...
    from ultrasync.keys import file_key, mem_key, sym_key
...
def anchors_show(ctx: click.Context, anchor_type: str):
    ...
    if not ps:
        ...
        click.echo("use 'ultrasync anchors list' to see available types")
        sys.exit(1)
...
@click.option(
    "--sort",
    "sort_by",
    type=click.Choice(["none", "name", "source"]),
    default="none",
    help="Sort output by name or source file (default: none)",
)
@click.pass_context
def ir_extract(
    ctx: click.Context,
    ...
    if output_format == "yaml":
        ir_dict = app_ir.to_dict(include_sources=sources, sort_by=sort_by)
        result = yaml.dump(ir_dict, default_flow_style=False, sort_keys=False)
    elif output_format == "json":
        ir_dict = app_ir.to_dict(include_sources=sources, sort_by=sort_by)
        result = json.dumps(ir_dict, indent=2)
    elif output_format == "markdown":
        result = app_ir.to_markdown(include_sources=sources, sort_by=sort_by)
    else:  # summary
...
def completion(shell: str):
    ...
    \b
    Bash:
        ultrasync completion bash >> ~/.bashrc

    \b
    Zsh:
        ultrasync completion zsh >> ~/.zshrc

    \b
    Fish:
        ultrasync completion fish > \
        ~/.config/fish/completions/ultrasync.fish
    """
    ...
    comp = comp_cls(cli, {}, "ultrasync", "_GALAXYBRAIN_COMPLETE")
